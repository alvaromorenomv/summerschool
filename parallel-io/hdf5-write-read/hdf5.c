...
void h5_writer(int my_id, int *localvector, int localsize)
{
    ...
    file_id = H5Fcreate("sos_bobo.h5", H5F_ACC_TRUNC, H5P_DEFAULT, plist_id);

    /* Create the dataset */
    filespace = H5Screate_simple(1, &dims, NULL);
    dset_id = H5Dcreate(file_id, "sos_bobo", H5T_NATIVE_INT, filespace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    H5Dwrite(dset_id, H5T_NATIVE_INT, memspace, filespace, H5P_DEFAULT, localvector);
...
void h5_reader(int my_id, int *localvector, int localsize)
{
    ...
    plist_id = H5Pcreate(H5P_FILE_ACCESS);
    H5Pset_fapl_mpio(plist_id, MPI_COMM_WORLD, MPI_INFO_NULL);
    file_id = H5Fopen("sos_bobo.h5", H5F_ACC_RDONLY, plist_id);
    H5Pclose(plist_id);
    /* Open the dataset and get the filespace id */
    dset_id = H5Dopen(file_id, "sos_bobo", H5P_DEFAULT);
