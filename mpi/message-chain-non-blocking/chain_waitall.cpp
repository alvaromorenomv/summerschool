...
int main(int argc, char *argv[])
{
    ...
    MPI_Isend(&message[0],size,MPI_INTEGER,destination,myid+1,MPI_COMM_WORLD,&request[0]);
    printf("Sender: %d. Sent elements: %d. Tag: %d. Receiver: %d\n",
           myid, size, myid + 1, destination);

    MPI_Irecv(&receiveBuffer[0],size,MPI_INTEGER,source,myid,MPI_COMM_WORLD,&request[1]);
