...
void evolve(Field& curr, Field& prev, const double a, const double dt)
{
  ...
  int field_size = (curr.nx+2) + (curr.ny+2);

  // Determine the temperature field at next time step
  // As we have fixed boundary conditions, the outermost gridpoints
  // are not updated.
  #pragma omp target teams distribute parallel for collapse(2) map(tofrom:currdata[0:field_size],prevdata[0:field_size])
...
int field_size = (curr.nx + 2) * (curr.ny + 2);
 ...
 #pragma omp target enter data map(to:prevdata[0:field_size]) map(to:currdata[0:field_size])
}
...
#pragma omp target exit data map(from:prevdata[0:field_size]) map(from:currdata[0:field_size])
}

void update_host(Field& field){
 double* fieldData = field.temperature.data();

 int field_size = (field.nx + 2) * (field.ny + 2);

 #pragma omp target update from(fieldData[0:field_size])
}
